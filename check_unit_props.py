    ...
    if hasattr(event, 'unit_controller') and event.unit_controller == player:
        if event.name in ["UnitInitEvent", "UnitBornEvent"]:
            if "Command" in event.unit.name or "Planetary" in event.unit.name:
                unit = event.unit
                print(f"[{event.second//60}:{event.second%60:02d}] {event.name}: {unit.name}")
                print(f"   Unit ID: {unit.id}")
                print(f"   is_building: {getattr(unit, 'is_building', None)}")
                print(f"   type_class: {getattr(getattr(unit, '_type_class', None), 'name', None)}")
                print(f"   event_unit_type: {getattr(event, 'unit_type_name', None)}")
