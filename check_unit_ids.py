...
filepath = r"C:\Users\pakow\Documents\StarCraft II\Accounts\57638796\1-S2-1-498796\Replays\Multiplayer\Magannatha LE (88).SC2Replay"
...
for event in replay.events:
    if hasattr(event, 'unit_controller') and event.unit_controller == player:
        if event.name in ["UnitInitEvent", "UnitBornEvent"]:
            if "Command" in event.unit.name or "Planetary" in event.unit.name or "Hatch" in event.unit.name or "Nexus" in event.unit.name:
